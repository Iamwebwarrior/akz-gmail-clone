    ...
    useEffect(()=>{
            db.collection("emails").orderBy("timestamp","desc").onSnapshot(snapshot=>{
                setEmails(snapshot.docs.map(doc=>({
                    id:doc.id,  //hre only two ver is extrected from firebase(id and data array form)
                    data:doc.data()
                })))
            })
    },[])
...
{
    emails.map(({id, data})=>{  //here we are using the ustate  data from firebase data
                                //extrected(id and data)
